...
AmountSet asCreate(CopyASElement copyElement,
    ...
    if(first_node == NULL){
        #ifndef NDEBUG
        printf("amount set creation problem");
        #endif
        free(set);
        return NULL;
    }
...
AmountSetResult asDelete(AmountSet set, ASElement element){
    ...
    //points to first "FAKE" node
    while(set->element_compare(element,current) != 0){
        previous = current;
...
AmountSet asCopy(AmountSet set){
    ...
    asChangeAmount(new_set, set->iterator, set->iterator->amount); //copy amount
    ...
    while(set->iterator->next != NULL){
        ...
        asChangeAmount(new_set, set->iterator, set->iterator->amount);
    }
    return new_set;
}